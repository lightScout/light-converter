...
const GLASS_FRAG = /* glsl */ `
  uniform vec3 uCam; uniform vec3 uLight; uniform float uTime;
  varying vec3 vN; varying vec3 vW;
  void main(){
...
export function mountHero(canvas: HTMLCanvasElement, opts: { scrollEl?: HTMLElement } = {}) {
  ...
  const glassGeo = new THREE.CylinderGeometry(R, R, LEN, 3, 1, false).toNonIndexed(); glassGeo.computeVertexNormals();
  const caps = new THREE.Mesh(glassGeo, new THREE.ShaderMaterial({ vertexShader: GLASS_VERT, fragmentShader: GLASS_FRAG, uniforms: glassU, transparent: true, depthWrite: false, blending: THREE.AdditiveBlending, side: THREE.DoubleSide }));
  const edges = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.CylinderGeometry(R, R, LEN, 3, 1, false), 10), new THREE.LineBasicMaterial({ color: 0xe6eeff, transparent: true, opacity: 0.55, blending: THREE.AdditiveBlending, depthWrite: false }));
  body.add(caps, edges);
  body.rotation.x = -Math.PI / 2;                  // apex up, faces left/right, length into the screen
  ...
  const frame = (now: number) => {
    ...
    const beamOn = 1 - smooth(0.32, 0.62, p);
